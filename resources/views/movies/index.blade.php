...
<div class="mobile">
    ...
    <div class="content" id="movies-container"></div>
</div>
...
<script>
    ...
    async function likeMovie(kinopoiskId) {
        try {
            const response = await reactMovie(kinopoiskId, 'like');
    ...
    async function reactMovie(kinopoiskId, reaction) {
        ...
        const response = await fetch(`{{ route('movie.react', ['session' => $session]) }}`, {
            method: 'POST',
            headers: {
